package ricardombertani.projetos.allinyourhands.core.apiresponse;
...
import ricardombertani.projetos.allinyourhands.apidata.directions.Direction;
import ricardombertani.projetos.allinyourhands.apidata.directions.Distance;
import ricardombertani.projetos.allinyourhands.apidata.directions.Duration;
import ricardombertani.projetos.allinyourhands.apidata.directions.Leg;
import ricardombertani.projetos.allinyourhands.apidata.directions.Location;
import ricardombertani.projetos.allinyourhands.apidata.directions.Route;
...
import ricardombertani.projetos.allinyourhands.apidata.lyrics.LyricCollection;
...
import ricardombertani.projetos.allinyourhands.apidata.song.Artist;
import ricardombertani.projetos.allinyourhands.apidata.song.ArtistCollection;
import ricardombertani.projetos.allinyourhands.apidata.song.Category;
import ricardombertani.projetos.allinyourhands.apidata.song.CategoryCollection;
import ricardombertani.projetos.allinyourhands.apidata.song.Song;
import ricardombertani.projetos.allinyourhands.apidata.song.SongCollection;
...
public class ResponseFormater {
	...
	public static BooksVolumeCollection formaterBooks1API_response(String response){ // resposta em JSON
	...
	public static String formaterSalesAPI_response(String response){ 
		...
		//log.debug("--> RESPONSE FORMATADO: "+response);
	...
	public  static String formaterDirectionsAPI_response(String response){ // usamos XML
		...
		/*if((distanceKm > 6) && (distanceKm < 10)){
			zoomValue = 13;
		}else if((distanceKm > 10) && (distanceKm < 60)){
			zoomValue = 12;
	...
	public static String formaterWeatherAPI_response(String response, String finalAdress){	
		...
		try {
			...
			if(isNotNullOrEmptyThisArray(list)){
				...
				log.debug("--> Temperature Integer Part :"+tempFieldValueIntPart+" In Celcius: "+tempInCelcius);
		...
		try {
			...
			if(isNotNullOrEmptyThisArray(songsObject)){
					...
					for(int i=0; i< songsObject.length(); i++){
						...
						methodParameters.put("songID", song.getId());
						methodParameters.put("country", "pt");							
						String teste = JGroove.callMethod(methodParameters, "getStreamKeyStreamServer");
						log.debug("\n---> STREAMING RESULT:  "+teste);*/
	...
	public static Lyric formaterLyricsAPI_response(String response){
		...
		//log.debug("---> Printing the response:  "+response);
