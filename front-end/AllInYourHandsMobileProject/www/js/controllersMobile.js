                ...
                function() {    
                  ...
                  for(i=0;i< $scope.recommendedContents.length;i++){
                          ...
                          if($scope.recommendedContents[i].image == 'images/2.png'){
                            $scope.recommendedContents[i].image = 'http://tudoemsuasmaos.com.br/images/2.png';
                          }
...
$scope.getSongsByQuery = function(){
  ...
  var Results = songService.get(
    {query:$scope.songName,pagenumber:$scope.songPageNumber,lyric:'0',grooveshark:true}, 
    function() {                  
...
$scope.getSongsByQuery();
            ...
            $scope.getArtistsByCategory = function(){
              ...
              var Results = artistsByCategoryService.get(
                {name:$scope.songCatName,pagenumber:$scope.artistsCatPageNumber,webVersion: false},
                function() {                  
...
$scope.getStreamingURL = function(){
  ...
  var Results = streamingUrlService.get(
    {songid:$scope.songID, ipadress: $scope.ipAdress, streamsessionid:$scope.statusAPIs.streamingSessionID},
    function() {    
 ...
 if($scope.videoNextPageTok == null){
  $scope.videoNextPageTok = ' ';
               ...
               var Results = videoPaginationService.get(
                {query:vQuery, categoryid: $scope.videoCatID, nextpagetoken:$scope.videoNextPageTok},
                function() {    
          ...
          $scope.getBooks = function(){
            ...
            var Results = bookService.get(
              ...
              function() {    
                ...
                if($scope.isReadingTheBook == true) {                                             
                       
                       $scope.isReadingTheBook = false;
                ...
                if(angular.isArray($scope.books)){
                  ...
                  for(i = 0; i< $scope.books.length; i++){
                   if($scope.books[i].volumeInfo.imageLink.extraLarge != ''){
                    ...
                    $scope.books[i].volumeInfo.imageLink.extraLarge = 'http://tudoemsuasmaos.com.br/images/booknail.jpg';
                  }
...
$scope.getBooksPreviousPage = function(){
  ...
  $scope.getBooks();
          ...
          $scope.getBooksNextPage = function(){
            ...
            $scope.getStates = function(countryIDandName){


              $scope.selectedCountryID = countryIDandName.split("|")[0];
              $scope.selectedCountryName = countryIDandName.split("|")[1];

              var Results = statesService.get( 
                {countryid:$scope.selectedCountryID},                                   
                function() {    

                 document.getElementById("countrySpanPlace").innerHTML=$scope.selectedCountryName;
                 document.getElementById("countrySpanWeather").innerHTML=$scope.selectedCountryName;						
...
$scope.updateFinalAddress = function(selectedStateName){

 document.getElementById("stateSpanPlaces").innerHTML=selectedStateName;	
 document.getElementById("stateSpanWeather").innerHTML=selectedStateName;	
 $scope.stateName = selectedStateName;
 $scope.addressValue = $scope.selectedCountryName +', '+selectedStateName;
...
$scope.getPlaces_ = function(){
  ...
  $scope.streetName = document.getElementById("placeNameID").value;
...
$scope.scroolToPosition = function(elementID){
          ...
          $scope.getWeather = function(){
            ...
            $scope.streetName = document.getElementById("weatherStreetNameID").value;
                  ...
                  function() {    
                    ...
                    if($scope.weather != null){
                      ...
                      if( ($scope.weather.weatherDescription.toLowerCase().indexOf("chuva") > -1) || ($scope.weather.weatherDescription.toLowerCase().indexOf("rain") > -1) ){
                        $scope.weather_forecast_image = "http://tudoemsuasmaos.com.br/images/rainy.png"
                      }else{
                       $scope.weather_forecast_image = "http://tudoemsuasmaos.com.br/images/sun.png"
                     }
...
$scope.getWeatherForecast = function(){
        ...
        $scope.playAction = function (songIDandURL, songName,songArtist, imagePreview) {
          ...
          if( (imagePreview == 'images/play_icon1.png') || (imagePreview == '')){
            imagePreview = 'http://tudoemsuasmaos.com.br/images/2.png';
          }
       ...
       var timer = buzz.toTimer( time );
          ...
          $scope.openSuggestedBook = function(id,name,type,artist,extraInformation,image){
            ...
            if(image.indexOf("openlibrary") != -1){
              ...
              // $scope.openedBook = window.open(  $scope.replaceAll(extraInformation,'*','/') );
              extraInformation = $scope.replaceAll(extraInformation, 'height=97%', 'height=900px' );
              document.getElementById("recommendedBookReadingArea").innerHTML = $scope.replaceAll(extraInformation,'*','/');

            }else{        


              bookHtml = "<html><header></header><body><iframe frameborder=\"0\" scrolling=\"yes\" style=\"border:0px\" src=\""+$scope.replaceAll(baseUrl,'\\*','/')+
              "&hl="+h1Value+"&lpg=PP1&pg=PP1&output=embed"+"\" width=97% height=900px;></iframe></body></html>";


              //$scope.openedBook = window.open("_blank");
              //$scope.openedBook.document.write(bookHtml);             
              document.getElementById("recommendedBookReadingArea").innerHTML += bookHtml;

              

            }

            $scope.isReadingTheBook = true;
          ...
          $scope.openBook = function(id,image,name,artistName,bookHtml,extraInfo) {
            ...
            bookHtml  = $scope.replaceAll(bookHtml, 'height=97%', 'height=900px' ); 
       
            document.getElementById("bookReadingArea").innerHTML += bookHtml;

            $scope.isReadingTheBook = true;
            
            /*if(image.indexOf("openlibrary") != -1){
               $scope.openedBook = window.open(bookHtml); // bookHtml no caso do openlibrary eh um link direto
        ...
        function onDeviceReady(){
          ...
          $('#booksModal').on('hidden.bs.modal', function () {             
           $("#bookModalBody" ).empty();
         });
